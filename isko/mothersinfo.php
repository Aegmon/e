...
if(isset($_POST['create'])){
        ...
        $con->query("INSERT INTO `mother`(`scholarID`, `mfname`, `mlname`, `mnumber`, `mdob`, `medu`, `mincome`, `ms_income`, `moccupation`, `maddress`) 
		VALUES('$user_check','$fname','$lname','$number','$dob','$edu','$income','$source','$occupation','$address')");
        $info_qry = "SELECT * from mother where scholarID='$user_check'";
        $info_ses = mysqli_query($con,$info_qry);
        $info = mysqli_fetch_assoc($info_ses);
        $infoid = $info['scholarID'];
        header("location: fatherinfo.php?scholarID=".$infoid);
}
?>
<!DOCTYPE html>
<html lang="en">
