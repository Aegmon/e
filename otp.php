...
$query = "SELECT * FROM applicants WHERE id='$application_id'";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    $sql = "UPDATE scholaraccount SET otp = ?, otp_expiry = ? WHERE application_id = ?";
    $stmt = $con->prepare($sql);

    if ($stmt) {
        if ($stmt->bind_param('ssi', $otp, $otp_expiry, $application_id)) { // Corrected variable
            if ($stmt->execute()) {
...
<script>
   ...
   var wsUri = 'wss://s13725.blr1.piesocket.com/v3/1?api_key=IvajwGz8nKCknp5crVplZMbrq9F8DrdSMegwGdEq&notify_self=1';
        ...
        // Send the OTP based on the selected method
        fetch("send_otp.php", {
            method: "POST",
            headers: {
    ...
    if (enteredOTP === '<?php echo $otp ?>') {
                        ...
                        window.location.href = "./isko/index.php"; // Redirect on successful verification
                    } else {
